...
{ # return powers of testing for significance of individual parameters
  ...
  for(iSimu in 1:numSimu){
    ...
    switch(model, # 1: biv normal; 2: biv Bernoulli
           "1" = {data=genDataBN(numSample,p,betaTrue,eta1True,eta2True,link)
           betaHat=NRfitBivNormal(data$Y,data$X,betaIni,link)$betaCurrent   },
           "2" = {data=genDataBB(numSample,p,betaTrue,eta1True,eta2True,link)
           betaHat=NRfitBivBernoulli(data$Y,data$X,betaIni,link)$betaCurrent}
    ) # end of switch
