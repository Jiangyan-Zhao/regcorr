#' Bivariate Bernoulli Correlation Regression Model
#'
#' Fits a correlation regression model for bivariate binary responses using S3 methods.
#'
...
#' @param beta0 Initial vector for beta coefficients. Default is NULL (zero vector).
#' @param nboot Number of bootstrap iterations for standard error estimation. Default is 100.
#' @return An object of class \code{"regcorr_binary"} and \code{"regcorr"}.
#' @export
binary <- function(formula, data = NULL, link = c("logistic", "tanh", "1", "2"), 
                   beta0 = NULL, nboot = 100) {
  call <- match.call()
  ...
  if (is.null(Y) || ncol(Y) != 2) {
    stop("The response variable must be a 2-column binary matrix, e.g., cbind(y1, y2) ~ x1 + x2")
  }
  ...
  # 3. 拟合点估计
  fit <- NRfitBivBernoulli(Y = Y, X = X, beta0 = beta0, link = link_code)
  ...
  if (nboot > 0) {
    ...
    for (i in 1:nboot) {
      ...
      boot_fit <- NRfitBivBernoulli(Y = Y[idx, ], X = X[idx, , drop = FALSE], 
                                    beta0 = beta0, link = link_code)
  ...
  class(res) <- c("regcorr_binary", "regcorr")
  return(res)
}